...
export default class extends Ob {
    ...
    setupUniforms() {
        ...
        if (this.fragType === "book") {
            uniforms.amount = {
                value:
                    uniforms.progress.value * (MAX_AMOUNT - MIN_AMOUNT) +
                    MIN_AMOUNT,
            };
            uniforms.cylinderCenter = { value: uniforms.amount.value };
            // 360 degrees * uniforms.amount
            uniforms.cylinderAngle = {
                value: 2.0 * Math.PI * uniforms.amount.value,
            };
            ...
            Object.defineProperty(uniforms.progress, "value", {
                set: function (newValue) {
                    ...
                    uniforms.cylinderCenter.value = uniforms.amount.value;
                    // 360 degrees * uniforms.amount
                    uniforms.cylinderAngle.value =
                        2.0 * Math.PI * uniforms.amount.value;
                    uniforms.cylinderRadius.value = 1.0 / Math.PI / 2.0;
                },
                get: function () {
                    return this._value;
    ...
    setupTexes(uniforms) {
        uniforms.texCurrent = {
            value: this.texes.get(`${config.prefix.tex}1`),
        };
    ...
    render(tick) {
        ...
        if (this.fragType === "book") {
            this.uniforms.amount.value =
                this.uniforms.progress.value * (MAX_AMOUNT - MIN_AMOUNT) +
                MIN_AMOUNT;
        }
    }
    afterInit() {
